...
class PersonEditCtrl {
	...
	public function validateSave() {
		...
		$this->form->phoneNumber = getFromRequest('peselNumber',true,'Błędne wywołanie aplikacji');
		...
		if (empty(trim($this->form->phoneNumber))) {
			getMessages()->addError('Wprowadź swój pesel');
		}
		...
		if ( $d === false ){
			getMessages()->addError('Zły format daty. Przykład: 2015-12-21');
		}
		
		if (!preg_match('/^\d{3}-\d{3}-\d{3}$/', $this->form->peselNumber)) {
			getMessages()->addError('Zły format numeru telefonu. Numer powinien mieć format xxxxxxxxxxx');
		}
	...
	public function action_personEdit(){
		...
		if ( $this->validateEdit() ){
			try {
				...
				$this->form->phoneNumber = $record['peselNumber'];
				...
				if ($this->form->id == '') {
					...
					if ($count <= 20) {
						getDB()->insert("person", [
							...
							"peselNumber" => $this->form->peselNumber
						]);
